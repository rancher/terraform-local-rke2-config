...
func SetAcmeServer() string {
	...
	if acmeserver == "" {
		// Safe to ignore error because the key "ACME_SERVER_URL" is strictly valid.
		_ = os.Setenv("ACME_SERVER_URL", "https://acme-staging-v02.api.letsencrypt.org/directory")
	}
	return acmeserver
}
...
func Teardown(t *testing.T, directory string, options *terraform.Options) {
	...
	if directoryExists {
		terraform.DestroyContext(t, t.Context(), options)
