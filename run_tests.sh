...
Invalid option -$OPTARG, valid options are
  ...
  -s to run tests in slow mode (one at a time to avoid rate limiting)
...
if [ "$slow_mode" = true ]; then
  echo "Running in slow mode: tests will be run one at a time to avoid rate limiting."
else
...
EOF
  ...
  # By setting both to 1, we ensure that tests are run sequentially, which can help avoid rate limiting issues.
...
if [ -z "$cleanup_id" ]; then
  ...
  # Clear root module locks and re-initialize
  (cd "$REPO_ROOT" && rm -rf .terraform .terraform.lock.hcl && terraform init -upgrade > /dev/null 2>&1 || true)
